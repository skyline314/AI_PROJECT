...
def process_frames():
    ...
    while True:
        ...
        try:
            if not pause_event.is_set():
                ...
                # if last_known_visuals["alert"]:
                #     cv2.putText(frame, "!!! PERINGATAN KANTUK !!!", (int(w/2) - 250, int(h/2) - 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)
                #     cv2.putText(frame, f"Penyebab: {last_known_visuals['alert_reason']}", (int(w/2) - 150, int(h/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        except Exception as e:
    ...
    # cap.release()
    # print("ℹ[Stream Thread] Kamera dilepaskan.")
